 ...
 */
public class OrdenDeVentaController implements Initializable {
    ...
    @FXML
    private SearchableComboBox<Menu> cboPlatillo2;
    ...
    private void llenarCombos() {
        ...
        //Llena combo de platillos-menu
        lstMenus = menudao.getAllMenuVig();
        //eliminamos el combo original de javafx y colocamos uno de la libreria
        //controlsFx (cboPlatillo2)
        //cboPlatillo.setItems(lstMenus);
        cboPlatillo2.setItems(lstMenus);
    }
    ...
    private void btnAgregar(ActionEvent event) {
        if (validar()) {
            ...
            double importe = Math.round((cantidad * cboPlatillo2.getValue().getPrecioMenu()) * 100.0) / 100.0;
            ...
            lstOrdenes.add(new OrdenTemporal(numMesa, cantidadClientes, Integer.parseInt(chbPersonas.getValue().toString()),
                    cboPlatillo2.getValue().getIdMenu(), cboPlatillo2.getValue().getDescripcionMenu(),
                    cboPlatillo2.getValue().getPrecioMenu(), cantidad, importe, cboTurno.getValue().getIdTurno(),
    ...
    private void c_btnProcesarOrden(ActionEvent event) {
        ...
        if (guardo) {
           // Validaciones.alertConfirm("Guardar", "Orden en Proceso");
            Stage stage = (Stage) btnProcesarOrden.getScene().getWindow();
            stage.close();
        } else {
